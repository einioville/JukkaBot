...
- **A stalled stream wedges the player thread, and only killing FFmpeg frees it.** FFmpeg has no socket timeout by default (the `tcp` protocol's own `-timeout` is `-1`), so a CDN that accepts the connection and then stops sending leaves it blocked in `recv()` — alive, silent, never exiting. `AudioPlayer._do_run` then parks in a blocking read on FFmpeg's stdout and only rechecks its stop event *between* frames, so `voice.stop()` sets a flag nothing ever looks at again: pause/skip/loop all log normally and do nothing, `is_playing()` stays stuck (which also blocks `idle_disconnect`), `after` never fires, and only a disconnect recovers. Two defences, both needed: `-rw_timeout` (`FFMPEG_RW_TIMEOUT_MICROSECONDS`) bounds the read so the stall becomes an ordinary non-zero exit the retry path handles, and `playback_watchdog` kills FFmpeg when `AudioPlayer.loops` (20ms frames, so ~50/sec while audio flows) sits still for `PLAYBACK_STALL_SECONDS`. The watchdog keys off the *live player thread*, never `is_playing()` — a stall after a skip has `_end` set already, so it reads as not playing while staying just as wedged. Killing beats `source.cleanup()`, which sets `_stopped` and makes the exit look intentional.
